...
//                return wavd;
  ...
  Recorder.get = function (callback, config) {  
    //   if (callback) { 
    //       window.alert(JSON.stringify(navigator.getUserMedia))
    ...
    //   } 
    if(callback) {
        window.alert(JSON.stringify(navigator.mediaDevices))
        // 老的浏览器可能根本没有实现 mediaDevices，所以我们可以先设置一个空的对象
        if (navigator.mediaDevices === undefined) {
            navigator.mediaDevices = {};
        }
        window.alert(JSON.stringify(navigator.mediaDevices.getUserMedia))

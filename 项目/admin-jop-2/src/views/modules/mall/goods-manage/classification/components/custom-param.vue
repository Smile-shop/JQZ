<template>
  <div class="classification-set">
    <title-header titleHeader="商品属性"></title-header>
    <div class="class-set-box">
      <p class="class-set-title">基本属性：</p>
      <p>说明：商品的基本属性；最多添加10个。</p>
      <a-button type="primary" class="class-set-button" @click="addModal">添加属性</a-button>
    </div>
    <!-- 表格 -->
    <div class="class-table class-table2">
        ...
        <!-- 详情页规格属性 -->
          ...
          >
            <div :key="col">
              <template v-if="record.editable">
                <a-switch default-checked @change="onSwitchChange" :checked="(record.visible === 'Y')"/>
              </template>
              <template v-else>
                <p>{{record.visible=="Y"?"开启":"隐藏"}}</p>
              </template>
            </div>
          </template>
          <!-- 自定义参数 -->
          ...
          >
            <div>
              <template v-if="record.editable&&((edtype==1&&!(data&&data.use))||edtype!=1)">
                <a-select style="width: 130px" v-model="record.datatype">
...
<script lang="ts">
import { defineComponent, reactive, ref, onMounted, watch } from '@vue/composition-api'
import Sortable from 'sortablejs';
import titleHeader from '../../product-list/components/search-form/title-header.vue'
...
export default defineComponent({
  ...
  setup(props, context) {
    onMounted(() => {
      if (props.data) {
        initData.tableData = Object.assign([], props.data.basePropertyList);
    ...
    const popDelectMethod = (key: any) => {
      if (props.data && props.data.use && props.edtype === 1) {
        context.root.$message.warning('该属性已与商品关联，不能删除');
        return false;
      }
      ...
      initData.comfirms.content = '是否删除已选基本属性？';
    }
    // 编辑
    const edit = (key: any) => {
      const newData = [...initData.tableData];
    ...
    const addModal = () => {
      if (props.data && props.data.use && props.edtype === 1) {
        context.root.$message.warning('该属性已与商品关联，不能新增');
        return false;
      }
      if (initData.tableData.length > 9) {
        context.root.$message.warning('商品属性最多添加10个');
        return false;
      }
      initData.parentData.modalVisible = true;
      initData.parentData.title = '添加基本属性';
      initData.parentData.modalType = '1';
      initData.parentData.itemTitle = '基本属性名称：';
      initData.parentData.mes = '基本属性';
    }
    // 货品属性

<template>
  <div class="received-message">
          ...
          <div v-else-if="replayContent[msgType]">
            <Popover>
              <div slot="show-content" v-if="msgType === 'news'">
                <div
                  class="news-box-content"
                  v-for="(item, index) in replayContent[msgType].wxContents.news_item"
                  :key="item.thumb_media_id"
                >
              ...
              <div slot="show-content" v-if="msgType === 'voice'">
                <div class="voice-box-content">{{replayContent[msgType].wxContents.name}}</div>
              </div>
    ...
    <PoupBox width="980" @cancel="graphicShow = false" v-model="graphicShow">
      <div slot="header" class="poup-box-header">
        {{computedGraphicName}}
        <span>( 素材来源于微信素材管理中心，如需要添加素材请前往微信公众平台素材管理 )</span>
      </div>
      <div slot="body" class="poup-box-body">
        ...
        <div class="voice-box-content" v-if="msgType === 'voice'">
          <div class="media-item" v-for="mediaItem in graphicList" :key="mediaItem.media_id">
            <el-radio
              class="name"
              v-model="selectedGraphicItem"
              :label="mediaItem"
            >{{mediaItem.name}}</el-radio>
...
export default class ReceivedMessage extends Vue {
      ...
      if (res.data.length > 0) {
        ...
        if (content) {
          (this.replayContent as any)[this.msgType] = {
            wxContents:
              this.msgType === 'text' ? '' : JSON.parse(content.wxContents),
  ...
  private selectGraphicOk() {
    const wxContents = this.computedGraphicContent(
      this.msgType,
      this.selectedGraphicItem,
      true
    );
    const content = this.computedGraphicContent(
      this.msgType,
      this.selectedGraphicItem,
      false
    );
  ...
  private submitReceivedMsg() {
    if (this.msgType === 'text') {
      ...
      const replayTextContent = (this.$refs.ue as any)
        .getContent()
        .replace(/<br\>/g, '</p><p>')
        .replace(/target="_blank"|&nbsp;|<p><\/p>| /g, '')
        .replace(/href/g, ' href');

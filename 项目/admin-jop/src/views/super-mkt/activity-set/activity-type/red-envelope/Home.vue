...
export default {
  ...
  methods: {
    ...
    showQRURL(item) {
      ...
      this.Linkvalue = `${VUE_APP_BASE_API}/wx-interface-web/wx/auth?companyKey=${user.serverUser.account.companyKey}&shareId=${data.id}&childPage=redenvelope&page=wx_game_group`
      const params = {
    ...
    toPage(page, item) {
      ...
      switch (page) {
        case 'set':
          this.$router.push({
            path: '/super-mkt/activity-set/activity-type/red-envelope/edit',
            query: {
        ...
        case 'data':
          this.$router.push({
            path: '/super-mkt/activity-set/activity-type/red-envelope/data',
            query: {
              id: item.id,
              jmpId: item.jmpId,
              name: item.activityName
    ...
    toAddPage() {
      this.$router.push(
        '/super-mkt/activity-set/activity-type/red-envelope/add'
      )
    },
    // 活动设置

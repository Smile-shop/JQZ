    ...
    <ul class="From-tab">
      ...
      <li class="info-col">
        ...
        <div class="row2">
          ...
          <el-radio :disabled="isPublishid" v-model="isNum" :label="gameNum">不限制总次数</el-radio>
        </div>
      </li>
      <li class="info-col">
...
export default {
  ...
  methods: {
    ...
    isMemberStatus($event) {
      if ($event == 0) {
        this.readonlyStatus = false
        this.mergeCheklist() // 查询上次选中在次数会员等级中有无存在
      } else {
        this.readonlyStatus = true
      }
    },
    mergeCheklist() {
      const list = Object.assign([], this.checkList)
      const flag = Object.assign([], this.memberGrade)
      const current = []
      list.map(item => {
      ...
      console.log('当前会员等级', flag)
      console.log('上一次选中的等级', list)
      console.log('合并后的等级', current)

      this.checkList = current
    },
    beginTimeLimit(event) {
      console.log(event)
    },
    endTimeLimit(event) {
      console.log(event)
    ...
    submitBasicSet() {
      ...
      if (Number(this.maxWinning) < 1) {
        this._Alert('中奖次数必须大于等于1', '中奖次数限制')
        return false
      }

      if (String(this.isNum) === NO_LIMIT) {
        this.form.totalNum = 0 // 游戏总次数不能传递 null 后台数据类型为 int
      }
    ...
    initQueryActive(obj) {
      queryActive(obj).then(res => {
        ...
        if (code == MARK && data) {
          this.isJmpGift = data.isJmpGift
          this.imgName = this.singleActiveData.imgName == null ? this.imgName : this.singleActiveData.imgName

      ...
      <el-table-column label="操作" min-width="420">
        <template slot-scope="scope">
          ...
          <el-button
            ...
            size="small"
            @click="$router.push({path: '/prestore/order-manage/orderList', query: { distributionId: scope.row.id, activityName: scope.row.title }})"
          >订单</el-button>
          <el-button
            class="btn"
            icon="el-icon-tickets"
            size="small"
            @click="$router.push({path: '/prestore/distri-manage/activity-user-list', query: { distributionId: scope.row.id, activityName: scope.row.title }})"
          >用户</el-button>
          <el-button
            class="btn"
            icon="el-icon-tickets"
            size="small"
            @click="$router.push({path: '/prestore/distri-manage/activity-list/channelPopularize', query: {distributionId: scope.row.id}})"
          >推广</el-button>
        </template>
      </el-table-column>
    </el-table>
...
export default class PrizeSetActivityTypeHome extends Vue {
  ...
  toPage(page: string, row: any) {
    switch (page) {
      case 'set':
        this.$router.push({
          path: '/prestore/distri-manage/activity-list/add',
          query: {
            id: row.id,
            status: row.status,
          },

<template>
  <div class="tableData">
    ...
    >
      ...
      <el-table-column label="会员卡号" min-width="120">
        <template slot-scope="scope">
      ...
      <el-table-column label="充值单号" min-width="120">
        <template slot-scope="scope">
          <div>{{ scope.row.indirectSuperiorName || '--' }}</div>
        </template>
      </el-table-column>
      <el-table-column label="礼品名称" min-width="120">
        <template slot-scope="scope">
          <div>{{ scope.row.indirectSuperiorName || '--' }}</div>
        </template>
      </el-table-column>
      ...
      <el-table-column label="领奖人姓名" min-width="130">
        <template slot-scope="scope">
          <div>{{ scope.row.shopName || '--' }}</div>
        </template>
      </el-table-column>
      <el-table-column label="领奖人手机" min-width="130">
        <template slot-scope="scope">
          <div>{{ scope.row.shopName || '--' }}</div>
        </template>
      </el-table-column>

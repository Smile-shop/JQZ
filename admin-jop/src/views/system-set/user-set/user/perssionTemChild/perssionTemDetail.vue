<template>
  <article class="permission-detail">
    <el-form label-width="100px">
        <el-form-item label="模板名称：" prop="loginName">
          <el-input style="width: 400px;" v-model="templateName" :maxlength="20" placeholder="模板名称, 最多20个字符" size="large"></el-input>
        </el-form-item>
    </el-form>
    ...
    <div class="edit-perssion-content">
      ...
      <div class="member-management-control" v-if="isMemberManagementIOS">
        ...
        <div class="member-management-content-1">
          <div>
            <el-checkbox v-model="MemberManagementIOSData.giveIntegral" true-label="1" false-label="0" size="medium" :disabled="MemberManagementIOSData.giveIntegral ? false : true">赠送积分</el-checkbox>
          ...
          <div>
            <el-checkbox v-model="MemberManagementIOSData.giveCoupon" true-label="1" false-label="0" size="medium" :disabled="MemberManagementIOSData.giveCoupon ? false : true">赠送优惠券</el-checkbox>
            <span class="info-item-2">
              设置每月可赠送优惠券张数：
        ...
        <div class="member-management-content">
          ...
          <div>
            <el-checkbox v-model="MemberManagementIOSData.queryShopFollowCollect" true-label="1" false-label="0" size="medium" :disabled="!MemberManagementIOSData.queryShopFollowCollect">查看门店跟进记录汇总</el-checkbox>
          </div>
          <div>
    ...
    <div class="submit-btn"><el-button size="small" @click="submitPermission" type="primary">保存</el-button><el-button size="small" @click="$emit('change', {comName: 'temList'})">返回</el-button></div>
  </article>
</template>
...
import { getAllChannelList, getAppsByChannel, getPermissionsTemplates, updatePermissionsTemplates } from '@/api/system-set';

@Component({
})
export default class PerssionEdit extends Vue {
  @Prop()
  permissionsTemplatesId!: string;
  ...
  private getAppsByChannel() {
    const params = {
      ...
      permissionsTemplatesId: this.permissionsTemplatesId
    };
  ...
  private getAccountAllPermission() {
    ...
    getPermissionsTemplates(params).then((res: any) => {
      this.templateName = res.data.templateName || this.templateName
      // 门店权限
  ...
  private submitPermission() {
    if (!this.templateName) {
      return this.$message.error('权限模板名称不能为空')
    }
    const params = {
      permissionsTemplatesId: this.permissionsTemplatesId,
  ...
  private toSubmitPermission(params: any) {
    updatePermissionsTemplates(params).then((res: any) => {
      this.permissionsTemplatesId = res.data.permissionsTemplatesId
      this.$message({
        type: 'success',
        message: '保存成功！'
      })

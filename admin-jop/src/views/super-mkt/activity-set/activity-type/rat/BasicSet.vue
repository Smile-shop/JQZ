<template>
  <div class="basicSet">
    <ul class="From-tab">
      <li class="info-col" v-if="fileList[0].url">
        <span class="lable">闪屏页图片：</span>
        <div class="row2">
          <el-upload
            class="upload-demo"
            action
          ...
          >
            ...
            <span slot="tip" class="el-upload__tip">最佳尺寸：750px*1334px，最佳大小：200KB内，不上传则不显示闪屏页</span>
          </el-upload>
        </div>
      </li>

      <li class="info-col" v-else>
        <span class="lable">闪屏页图片：</span>
        <div class="row2">
          <el-upload
            class="upload-demo"
      ...
      <li class="info-col">
        <span class="lable">游戏时间：</span>
        <div class="row2">
          <el-input min="10" max="90" :disabled="isPublishid" v-model="form.gameTime">
      ...
      <li class="info-col">
        <p class="timetip">注：至少10秒，上限90秒，请配合奖品线距离合理设置游戏时间</p>
      </li>

      <li class="info-col">
        <span class="lable">是否开启倒计时：</span>
        <div class="row2">
          <el-radio v-model="form.isCountdown" label="1">开启(默认3秒)</el-radio>
          <el-radio v-model="form.isCountdown" label="0">关闭</el-radio>
        </div>
      </li>
...
export default {
  ...
  data() {
    return {
      ...
      maxWinning: 1, // 这大中奖次数
      imgName: '', // 默认展示例图
      fileList: [{ name: '', url: '' }], // 图片上传成功
      form: {
        activeTheme: '', // 副标题
        imgName:
          'http://www.jqzjop.com/ftp_images/01/game/game_default_banner.png', // 广告图
        activityName: '红包大抽奖，大奖等着您',
        subName: '新人专享好礼，最大888元红包',
        jmpId: '130',
        id: '',
        gameTime: 30, // 游戏时长
        isCountdown: '1', // 是否开启倒计时
        isPublish: '0', // 发布情况
        beginTime: '', // 开始日期
        endTime: '', // 结束日期
        isPaused: '1', // 活动状态
  ...
  methods: {
      ...
      queryActive(Data)
        .then(res => {
          ...
          if (code == 0) {
            data.isPaused = '0'
            data.isCountdown = '1'
            this.fileList[0].url = data.imgName
    ...
    submitBasicSet() {
      ...
      if (this.form.gameTime < 10 || this.form.gameTime > 90) {
        this._Alert('游戏时间请设置在10-90秒', '游戏时间')
        return false
      }
      ...
      if (String(this.isNum) === LIMIT && Number(this.totalNum) < 1) {

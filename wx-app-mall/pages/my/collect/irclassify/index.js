...
Component({
  ...
  methods: {
    ...
    allOnChange() {
      this.setData({
        shopppingCartList: this.computedShoppingCartList(this.data.shopppingCartList, !this.data.allSelect),
        isValidList: this.data.isEdit ? this.computedShoppingCartList(this.data.isValidList, !this.data.allSelect) : this.computedShoppingCartList(this.data.isValidList, false),
    ...
    goodsDetail(e) {
      ...
      // 判断是否下架
      const isLost = goods.valid == 0 || goods.isNullStock == 1
      if (isLost) {
        wx.showToast({
          title: `商品${goods.valid == 0 ? '已失效' : '已下架'}`,
          duration: 2000,
          icon: 'none'
        })
      } else {

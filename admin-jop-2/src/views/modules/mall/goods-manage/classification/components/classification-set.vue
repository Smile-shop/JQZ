<template>
  <div class="classification-set">
    <div class="order-button">
      <a-button type="primary" @click="goCustom">添加自定义分类</a-button>
    </div>
    <p class="class-set-title">一级分类</p>
    <!-- 表格 -->
    <div class="class-table class-table1">
        <a-table
    ...
    <!-- 二级分类 -->
    <classification-set-two @edit-custom="editCustom"></classification-set-two>
  </div>
</template>
...
import classificationSetTwo from './classification-set-two.vue';
import {
...
export default defineComponent({
  components: {
    Sortable,
    classificationSetTwo
  },
  setup(props, context) {
    onMounted(() => {
      init();
      // 表格中需要实现行拖动，所以选中tr的父级元素
      const table = document.querySelector('.class-table1 .ant-table-tbody');
    ...
    const editCustom = (id:string, type:number) => {
      context.emit('custom-style', id, type);
    }
